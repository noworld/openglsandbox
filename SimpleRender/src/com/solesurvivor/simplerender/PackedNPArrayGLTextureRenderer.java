...
public class PackedNPArrayGLTextureRenderer implements GLSurfaceView.Renderer {

	private static final String TAG = PackedNPArrayGLTextureRenderer.class.getSimpleName();
	...
	public PackedNPArrayGLTextureRenderer(Context context) {
	...
	protected void drawScene() {
		...
		GLES20.glVertexAttribPointer(a_pos, V_DATA_SIZE, GLES20.GL_FLOAT, false, 0, mPosOffset);

		GLES20.glEnableVertexAttribArray(a_nrm);
		GLES20.glVertexAttribPointer(a_nrm, V_DATA_SIZE, GLES20.GL_FLOAT, false, 0, mNrmOffset);
...
//		System.arraycopy(tempMatrix, 0, mViewMatrix, 0, 16);
	...
	protected void loadModel() {
		...
		float[] combinedArray = new float[combinedSize];		
		mPosOffset = 0;
		mNrmOffset = mPositions.length;
		mTxcOffset = mPositions.length + mNormals.length;
		System.arraycopy(mPositions, 0, combinedArray, mPosOffset, mPositions.length);
		System.arraycopy(mNormals, 0, combinedArray, mNrmOffset, mNormals.length);
//		System.arraycopy(mTexCoords, 0, combinedArray, mTxcOffset, mTexCoords.length);
		
		mDataBuf = SSArrayUtil.arrayToFloatBuffer(BYTES_PER_FLOAT, combinedArray);
		/*New - one big array*/
		Log.d(TAG, "Models loaded.");
		
		/*Build the IBO*/
		mIndexes = new short[mPositions.length];
		...
		mTxcBufIdx = buffers[1];
		mIndexBufIdx = buffers[2];
		...
		mNumElements = mIdxBuf.capacity() / TRIANGLE_NUM_SIDES;
