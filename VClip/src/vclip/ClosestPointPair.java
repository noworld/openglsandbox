...
//  	 }
	...
	public boolean epsilonEquals (ClosestPointPair pair, double eps)
	 {
	   if (!pnt1.epsilonEquals (pair.pnt1, eps))
	    { return false;
	    }
	   if (Math.abs((pnt1.x - pair.pnt1.x) +
			(nrml.x - pair.nrml.x)*dist) > eps)
	    { return false;
	    }
	   if (Math.abs((pnt1.y - pair.pnt1.y) +
			(nrml.y - pair.nrml.y)*dist) > eps)
	    { return false;
	    }
	   if (Math.abs((pnt1.z - pair.pnt1.z) +
			(nrml.z - pair.nrml.z)*dist) > eps)
	    { return false;
	    }
	   return true;
	 }
